...
handle_run() {
    ...
    # Subtask 3.f.1
    # Create new layer
    # Create new directories for the new layer: upper, work, and diff (the merged directory)
    mkdir -p "$CACHEDIR/layers/$layer_hash"/{work,diff,temp}

    # Subtask 3.f.2
    # Temporarily mount the overlay filesystem
    # lowerdir is the parent layer (the existing layer stack)
    mount -t overlay overlay -o lowerdir="$parent_layers",upperdir="$CACHEDIR/layers/$layer_hash/diff",workdir="$CACHEDIR/layers/$layer_hash/work" "$CACHEDIR/layers/$layer_hash/temp"

    # Subtask 3.f.3
    # Execute the command in the new mount
    # Use chroot so that the command sees the merged (overlay) filesystem.
    chroot "$CACHEDIR/layers/$layer_hash/temp" bash -c "$command"
    
    # Subtask 3.f.4
    # Cleanup and record metadata
    # After command execution, unmount the temporary overlay.
    umount "$CACHEDIR/layers/$layer_hash/temp"   
    rmdir "$CACHEDIR/layers/$layer_hash/temp"   

    # Record metadata for this layer.
    echo "RUN $command" > "$CACHEDIR/layers/$layer_hash/metadata"
    echo "$parent_hash" > "$CACHEDIR/layers/$layer_hash/parent"
    
    # Update the current layer to the new layer.
    current_layer="$CACHEDIR/layers/$layer_hash"
...
handle_copy() {
    ...
    # Subtask 3.e.1
    # Create a new layer
    # Create directories for the new layer: upper, work, and diff (the merged directory).
    mkdir -p "$CACHEDIR/layers/$layer_hash"/{work,diff,temp}


    # Subtask 3.e.2
    # Temporarily mount the overlay filesystem
    # Here the lower directory is the parent layer (i.e. the layer stack from the previous image build or RUN)
    mount -t overlay overlay -o lowerdir="$parent_layers",upperdir="$CACHEDIR/layers/$layer_hash/diff",workdir="$CACHEDIR/layers/$layer_hash/work" "$CACHEDIR/layers/$layer_hash/temp"
    
    # Subtask 3.e.3
    # Copy the files from source to destination
    # Note: The destination path is relative to the overlay mount point.

    # Ensure the destination directory exists in the overlay directory.
    cp -a "$src" "$CACHEDIR/layers/$layer_hash/temp$dest"
    ...
    # Record metadata about this layer so that it becomes part of the image's layer stack.arent layer
    echo "COPY $src $dest" > "$CACHEDIR/layers/$layer_hash/metadata"
    echo "$parent_hash" > "$CACHEDIR/layers/$layer_hash/parent"

    # Set current_layer to the new layer.
    current_layer="$CACHEDIR/layers/$layer_hash"
    echo "$current_layer" > "$CACHEDIR/layers/.last_layer"
}
    ...
    if [ ! -d "$CACHEDIR/base/$BASE_NAME" ]; then
        ...
        case "$DISTRO" in
            debian|ubuntu)
                ...
                ;;
            archlinux)
                echo "=== ARCHLINUX BOOTSTRAP START ==="
                local BOOTSTRAP_URL="https://mirror.rackspace.com/archlinux/iso/latest/archlinux-bootstrap-x86_64.tar.zst"
                if [ ! -f /tmp/arch-bootstrap.tar.zst ]; then
                    curl -L "$BOOTSTRAP_URL" -o /tmp/arch-bootstrap.tar.zst || die "Failed to download Arch bootstrap tar"
                fi
                mkdir -p "$CACHEDIR/base/$BASE_NAME"
                tar -I zstd -xf /tmp/arch-bootstrap.tar.zst -C "$CACHEDIR/base/$BASE_NAME" --strip-components=1 || die "Failed to extract Arch bootstrap tarball"

                sed -i -e '/#Server = https:\/\/geo\.mirror\.pkgbuild\.com\/\$repo\/os\/\$arch/s/^#//' \
                       -e '/#Server = https:\/\/ftpmirror\.infania\.net\/mirror\/archlinux\/\$repo\/os\/\$arch/s/^#//' \
                       -e '/#Server = http:\/\/mirror\.rackspace\.com\/archlinux\/\$repo\/os\/\$arch/s/^#//' \
                       -e '/#Server = https:\/\/mirror\.rackspace\.com\/archlinux\/\$repo\/os\/\$arch/s/^#//' "$CACHEDIR/base/$BASE_NAME/etc/pacman.d/mirrorlist"

                echo "nameserver 8.8.8.8" > "$CACHEDIR/base/$BASE_NAME/etc/resolv.conf"

                echo "=== ARCHLINUX BOOTSTRAP COMPLETE ==="
                ;;
            *)
                die "Unsupported base distro: $DISTRO"
                ;;
        esac
    fi
...
#    cp -a "$IMAGEDIR/$IMAGE"/* "$CONTAINERDIR/$NAME/rootfs"
    ...
    # Create directories for overlayfs: upper, work, and merged
    mkdir -p "$CONTAINERDIR/$NAME"/{upper,work,merged}

    # Read the base layer from the image's 'layers' file
    local lowerdir
    lowerdir=$(cat "$IMAGEDIR/$IMAGE/layers")
    
    # Mount the overlay filesystem, which will serve as the container's rootfs.
    mount -t overlay overlay -o lowerdir="$lowerdir",upperdir="$CONTAINERDIR/$NAME/upper",workdir="$CONTAINERDIR/$NAME/work" "$CONTAINERDIR/$NAME/merged" 

    # Save the merged directory as the container's rootfs.
...
#    unshare --mount --uts --ipc --net --pid --fork --kill-child \
#        bash -c "chroot  $CONTAINER_ROOTFS/rootfs \
...
#        exec $INIT_CMD_ARGS\""
    ...
    unshare --mount --uts --ipc --net --pid --fork --kill-child \
        bash -c "chroot $CONTAINER_ROOTFS \
        bash -c \"mount -t proc proc /proc && \
        mount -t sysfs sys /sys && \
        chmod 755 / && \
        exec $INIT_CMD_ARGS\""
...
#    umount "$CONTAINERDIR/$NAME/rootfs/dev" > /dev/null 2>&1 || :
    ...
    # Subtask 3.d.4
    # Unmount the overlay filesystem
    # Unmount the overlay merged directory before removing the container directory
    umount "$CONTAINERDIR/$NAME/merged" > /dev/null 2>&1 || true
...
#    local UNSHARE_PID=$(ps -ef | grep "$CONTAINERDIR/$NAME/rootfs" | grep -v grep | awk '{print $2}')
    local UNSHARE_PID=$(ps -ef | grep "unshare" | grep "$CONTAINERDIR/$NAME/merged" | grep -v grep | awk '{print $2}')
    ...
    # The container's init process is a child of the unshare process
#    local CONTAINER_ROOTFS=$(realpath "$CONTAINERDIR/$NAME/rootfs")
    local CONTAINER_ROOTFS=$(realpath "$CONTAINERDIR/$NAME/merged")
    nsenter -a -t "$CONTAINER_INIT_PID" chroot "$CONTAINER_ROOTFS" $EXEC_CMD_ARGS
...
addnetwork() {
    ...
    # If the host-side interface already exists, delete it.
    if ip link show "$OUTSIDE_PEER" &>/dev/null; then
        ip link delete "$OUTSIDE_PEER" || true
    fi

    # Also, if the container-side interface exists in the host (unlikely, but just in case)
    if ip link show "$INSIDE_PEER" &>/dev/null; then
        ip link delete "$INSIDE_PEER" || true
    fi

    # Create the veth pair
    ip link add "$OUTSIDE_PEER" type veth peer name "$INSIDE_PEER"
    
    # Move the container end of the veth pair to the container's network namespace
    ip link set "$INSIDE_PEER" netns "$NSDIR"

    # Configures the interface by assigning it an IP address along with its subnet mask
